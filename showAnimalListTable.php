...
$page_url = 'http://www.stefanlindh.se/DjurMMORPG/showAnimalListTable.php';  // TODO: hard coded url, need to be changed and saved in config-file.
...
elseif ( !isset($_GET['location']) ) {	// no GET['location'] given, but Username set (i.e. user with an active session tried to reach index.php for some reason)
	$location = 'showAnimalListTable';				// will showAnimalListTable page
} else {
...
header("Link: <$page_url?location=showAnimalListTable>; rel=\"previous\"; title=\"UTF-8'se'Inloggad\"");
...
function changelocation($location) {
	switch ($location) {
		...
		case 'SouthAmericaAndeans':
			include ('animalchart/southAmericaAndeans.php');
			break;
		case 'TEMPCanada':
			include ('animalchart/canada.php');
			break;
		case 'AfricaMadagascar':
			include ('animalchart/africaMadagascar.php');
			break;
		case 'Australia':
			include ('animalchart/australia.php');
			break;
